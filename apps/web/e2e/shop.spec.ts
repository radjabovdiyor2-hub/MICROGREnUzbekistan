...
test.describe("Каталог", () => {
    ...
    test("поиск принимает ввод", async ({ page }) => {
        ...
        // Дожидаемся клиентской отрисовки товаров: это признак того, что React
        // догидратировал страницу. Без ожидания fill попадает в ещё «мёртвую»
        // разметку, гидратация затем возвращает управляемому полю значение из
        // состояния — пустую строку, — и ввод пропадает. Тест падал именно так,
        // причём через раз: успеет гидратация до fill или нет, зависело от
        // загрузки машины.
