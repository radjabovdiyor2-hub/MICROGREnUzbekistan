      ...
      // All pages — security headers + short cache for HTML
      {
        ...
        headers: [
          ...
          },
          {
            ...
            //
            // geolocation=(self), А НЕ (). Пустой список запрещал
            // геопозицию ВСЕМУ САЙТУ, включая собственные страницы, и
            // ломал это молча: браузер не бросает исключение, он просто
            // отвечает отказом в доступе. Из-за этого отметка визита все
            // это время уходила без координат — `readPosition` честно
            // возвращает null на отказ, и подтверждение места просто не
            // прикладывалось. Нашлось живой проверкой записи дня, а не
            // проверками: ни одна из них геопозицию не трогает.
            //
            // `(self)` — это по-прежнему запрет для чужих origin во
            // фреймах, то есть ровно та защита, ради которой заголовок и
            // ставили. Разрешение у человека браузер спросит сам.
