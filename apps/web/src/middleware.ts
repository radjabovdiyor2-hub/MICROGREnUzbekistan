...
const RULES: Rule[] = [
  ...
  // DELETE здесь же: продавец отменяет СВОЙ объезд (заболел, перенесли),
  // а чей именно — решает сам роут по подписи. Держать удаление за
  // владельцем значило бы, что отменённый выезд висит до вечера.
  // Подтверждение «Приступить» приходит от БОТА, а не из браузера:
  // у него нет сессии, только общий секрет. Правило стоит ВЫШЕ общего
  // по длине префикса, поэтому оно и выигрывает.
  ...
  // ежедневная работа, а не настройка системы.
  { prefix: '/api/admin/visit-schedules', access: 'STAFF', methods: ['GET', 'PUT'] },
  ...
  // Крошки трека шлёт сам полевой сотрудник — из Telegram или из PWA, — и
  // дверь открыта только на запись. Читать чужой день здесь нельзя: отчёт
  // о дне живёт под своим префиксом и остаётся владельцу.
  { prefix: '/api/admin/tracking/ping', access: 'STAFF', methods: ['POST'] },
  // Фотоотчёт делает тот, кто стоит у дверей, — значит продавец. Роут сам
  // проверяет, что стоянка существует: middleware о содержимом не знает.
  { prefix: '/api/admin/tracking/photo', access: 'STAFF', methods: ['POST'] },
  // СВОЯ смена: открыть, закрыть, узнать состояние. Соседняя
  // `/api/admin/shifts` — это график, и она остаётся владельцу; отметить
  ...
  { prefix: '/api/admin/shifts/close-forgotten', access: 'STAFF', methods: ['POST'] },
  // Отчёт дня. Здесь STAFF означает «свой день»: чей именно, решает сам
  // роут по имени из подписи — middleware владельца записи знать не может.
  // Тот же приём, что у рейсов доставки строкой выше.
  { prefix: '/api/admin/tracking/day', access: 'STAFF', methods: ['GET'] },
  // «Я на точке» и «уехал». Чья стоянка — роут решает по подписи и
  // закрыть чужую не даёт.
  { prefix: '/api/admin/tracking/stay', access: 'STAFF', methods: ['POST', 'PATCH'] },
  ...
  // Каналы продаж: приём заказов с площадок и разбор очереди обновлений.
  // Сюда ходят интегратор и планировщик с `BOT_SECRET` — тем же ключом,
  // что и остальные внешние вызовы. Дверь создаёт заказы, поэтому роут
  // проверяет подпись и сам: одного префикса в таблице мало.
  { prefix: '/api/channels', access: 'ADMIN' },
