...
export const SETTINGS = {
  // ── Поставки заведениям ─────────────────────────────────────────────
  //
  // ЭТИ ДВА ЧИСЛА ЗАКУПЩИК СПРАШИВАЕТ ПЕРВЫМИ, и до сих пор ответить на
  // них сайт не мог: на странице для ресторанов не было ни минимального
  // заказа, ни срока. Человек писал в форму, ждал менеджера и половину
  // случаев отваливался, не дождавшись.
  //
  // Они в НАСТРОЙКАХ, а не в коде: минимальный заказ меняется вместе с
  // загрузкой теплицы, и владелец должен уметь поправить его сам, не
  // дожидаясь выкатки. Объём и цену по-прежнему называет менеджер — их
  // здесь нет намеренно.
  'b2b.minOrder': {
    category: 'b2b', type: 'money', default: 0, publicKey: true,
    labelRu: 'Минимальный заказ для заведений', labelUz: 'Muassasalar uchun eng kam buyurtma',
    hintRu: '0 — минимума нет, и страница так и напишет. Не выдумывайте число: обещание на сайте придётся выполнять',
    min: 0,
  },
  'b2b.orderCutoffHour': {
    category: 'b2b', type: 'number', default: 18, publicKey: true,
    labelRu: 'До какого часа принять заказ на завтра', labelUz: 'Ertangi buyurtma uchun soat',
    hintRu: 'Заказ до этого часа попадает в завтрашнюю срезку. Показывается закупщику',
    min: 0, max: 23,
  },
  // ── Доставка ────────────────────────────────────────────────────────
  'delivery.fee': {
  ...
  // Норма заходов переводит сбыт из настроения в задачу. Без числа неделя
  // без единой новой двери выглядит так же, как неделя с пятью: обе
  // «работали», а разницу видно только через месяц по остановке роста.
  'kpi.weeklyNewVisits': {
    category: 'stock', type: 'number', default: 5,
    labelRu: 'Новых заходов в неделю, норма', labelUz: 'Haftada yangi tashriflar normasi',
    hintRu: 'Меньше этого числа новых заведений за неделю — сигнал', min: 0,
  },
  ...
  // День выплаты нужен не бухгалтерии, а платёжному календарю: без него
  // самая крупная регулярная выплата не попадала в прогноз кассы и разрыв
  // в неделю зарплаты был не виден до самого дня выплаты.
  'payroll.payday': {
    category: 'payment', type: 'number', default: 5, min: 1, max: 31,
    labelRu: 'День выплаты зарплаты', labelUz: "Oylik to'lanadigan kun",
    hintRu: 'Число месяца. Если в месяце нет такого числа (30-е в феврале), выплата сдвигается на последний день месяца, а не в следующий',
  },

  // ── Работа в поле ───────────────────────────────────────────────────
  //
  // Пороги, по которым трек превращается в утверждения о человеке. Именно
  // поэтому они настройки, а не константы в коде: в плотном центре и на
  // окраине «рядом» — это разные числа, и подгонять их придётся по факту,
  // а не пересборкой.
  'field.stayRadiusM': {
    category: 'field', type: 'number', default: 150, min: 30, max: 600,
    labelRu: 'Радиус «на точке», метры', labelUz: 'Nuqtadagi radius, metr',
    hintRu: 'Ближе этого к пину клиента — считаем, что человек у него. Совпадает с радиусом подтверждения визита. Меньше — честные заезды начнут отмечаться далёкими',
  },
  'field.minStayMin': {
    category: 'field', type: 'number', default: 5, min: 1, max: 120,
  ...
  'field.dayHalfMinutes': {
    category: 'field', type: 'number', default: 240, min: 60, max: 600,
    labelRu: 'Половина рабочего дня, минуты', labelUz: 'Ish kunining yarmi, daqiqa',
    hintRu: 'Столько от НАЧАЛА СМЕНЫ без единой отметки — и сотруднику уходит вопрос, а через круг сигнал вам. Считается от смены, а не по часам на стене: открыл в 11:00 — в 13:00 у него ещё не полдня',
  },
  ...
  'field.trackKeepDays': {
    ...
    hintRu: 'Сырой трек — рабочий материал, а не архив. Стоянки, фото и плечи остаются: удаляются только крошки',
  },
