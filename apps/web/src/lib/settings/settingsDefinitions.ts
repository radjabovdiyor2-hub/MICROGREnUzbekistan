...
export interface SettingDef {
  ...
  max?: number;
  /**
   * Для `list` — закрытый словарь допустимых значений.
   *
   * Без него в список можно вписать что угодно, и оно молча уедет в
   * публичный /api/config: так на оформлении оказались Click и Payme,
   * которых нет. Значение вне словаря — отказ, а не запись мусора.
   */
  allowed?: string[];
