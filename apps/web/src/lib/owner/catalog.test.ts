...
describe('каталог практик', () => {
  it('у каждой практики есть объяснение — НА ОБОИХ языках', () => {
    // Узбекская половина проверяется наравне с русской: перевод, забытый
    // у одной практики из двухсот семидесяти девяти, на экране выглядит
    // как пустая строка под заголовком, и найти его глазами нечем.
    const mute = PRACTICES.filter(
      (p) => p.why.ru.trim().length < 20 || p.why.uz.trim().length < 20,
    );
  ...
  it('у каждой практики есть заголовок на обоих языках и источник', () => {
    const bad = PRACTICES.filter(
      (p) => !p.title.ru.trim() || !p.title.uz.trim() || p.videos.length === 0,
    );
    expect(bad.map((p) => p.key)).toEqual([]);
  });

  it('узбекский заголовок не оставлен русским', () => {
    // Самый вероятный способ «перевести» двести семьдесят девять строк —
    // скопировать русскую в узбекскую и забыть. Кириллица в узбекском поле
    // это ловит, а глаз при таком объёме — нет.
    const cyrillic = PRACTICES.filter(
      (p) => /[А-Яа-яЁё]/.test(p.title.uz) || /[А-Яа-яЁё]/.test(p.why.uz),
    );
    expect(cyrillic.map((p) => p.key)).toEqual([]);
  });
