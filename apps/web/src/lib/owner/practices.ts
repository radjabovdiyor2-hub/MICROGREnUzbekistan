...
export interface Practice {
  key: string;
  /**
   * Название и обоснование — ПАРОЙ на двух языках, как у `AREA_LABELS`
   * ниже. Раньше здесь стояли простые строки, и сервер клал в ответ
   * русскую версию — то есть решал за читателя, на каком языке тот читает.
   *
   * `key` при этом остаётся неизменным: по нему хранится отметка практики
   * в базе, и переименование ключа обнулило бы историю владельца.
   */
