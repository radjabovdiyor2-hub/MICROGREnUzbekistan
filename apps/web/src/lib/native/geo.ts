...
interface BackgroundGeolocationPlugin {
  ...
  /** Открыть настройки приложения — там живут разрешения и «Батарея». */
...
/**
 * Родная служба отдаёт КАЖДЫЙ замер — прореживает `shouldKeep`.
 *
 * Здесь стояло 25 метров, и это ломало контроль. Плагин передаёт число в
 * `setSmallestDisplacement`: пока человек стоит, служба не отдаёт НИ ОДНОЙ
 * точки. Двадцать минут у клиента становились «молчит» на карте владельца,
 * а простой дольше получаса для приложения не определялся никогда — окну
 * простоя было не на чем строиться.
 *
 * Прореживание осталось одно и общее с браузером: крошка раз в 45 секунд
 * или через 25 метров (`pingQueue.ts`). Стоящий телефон теперь даёт точку
 * раз в 45 секунд, и «молчит» снова значит «нет связи».
 *
 * ЦЕНА — ЗАРЯД. Служба будит страницу на каждый замер, лишнее отбрасывается
 * одной проверкой. Сам приёмник при этом работает и так: его держит
 * постоянное уведомление смены.
 */
 ...
 *
 * ЗАЧЕМ. Xiaomi, Redmi и Samsung по умолчанию усыпляют фоновые службы, и
 * постоянное уведомление их не спасает: трек рвётся на полчаса, а в коде
 * всё исправно. Выключается это только руками — «Батарея → Без
 * ограничений» в настройках приложения, куда кнопка и ведёт.
 *
 * `false` — мы не в приложении, открывать нечего.
 */
export function openNativeSettings(): boolean {
  if (!isNativeApp()) return false;
  const plugin = nativePlugin<BackgroundGeolocationPlugin>(PLUGIN);
  if (!plugin) return false;
  void plugin.openSettings().catch((error) => {
    console.error('[geo] настройки приложения не открылись:', error);
  });
  return true;
}
