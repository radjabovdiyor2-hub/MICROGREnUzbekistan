...
describe('summarize', () => {
  ...
  it('реальная частота Telegram даёт ненулевой путь', () => {
    // РЕГРЕССИЯ. Здесь стоял порог разрыва в пять минут, и живая проверка
    // на настоящей базе показала «пройдено 0 метров» за день с двумя
    // заездами и дорогой между ними: Telegram экономит батарею и при
    // ровном движении молчит шесть-семь минут, поэтому в длину пути не
    // попадал НИ ОДИН отрезок дороги. Из двух ошибок эта хуже завышения:
    // «прошёл ноль километров» выглядит как прогул.
    const drive = [ping(21 * 60), ping(14 * 60, 3000), ping(7 * 60, 6000), ping(0, 9000)];
    const { meters, gaps } = summarize(drive);
    expect(gaps).toBe(0);
