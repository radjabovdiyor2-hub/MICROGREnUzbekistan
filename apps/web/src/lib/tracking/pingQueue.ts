...
import { MAX_ACCURACY_M, MAX_BATCH, type PingSource } from './ping';
...
/**
 * Порог точности живёт в `ping.ts` — один на телефон и сервер.
 *
 * Здесь он проверяется ДО очереди, чтобы не копить и не отправлять
 * заведомо негодное; сервер проверяет ещё раз, потому что крошки Telegram
 * через эту очередь не проходят вовсе. Реэкспорт — чтобы проверка очереди
 * говорила тем же числом, что и сервер.
 */
