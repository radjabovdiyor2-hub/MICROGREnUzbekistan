 ...
 *
 * `pwa` и `app` РАЗВЕДЕНЫ НАМЕРЕННО. Вкладка браузера засыпает, когда
 * телефон в кармане, а родная служба Android пишет с погашенным экраном —
 * это два разных способа с разной плотностью точек. Пока оба назывались
 * `pwa`, вопрос «почему линия прямая» был неотвечаем по данным: редкие
 * точки одинаково выглядели и у спящей вкладки, и у приложения.
 *
 * Значение попадает в ключ `@@unique([employeeId, at, source])`, поэтому
 * переименовать его потом — значит развести старые строки с новыми.
 */
export const PING_SOURCES = ['telegram_live', 'pwa', 'app', 'visit'] as const;
 ...
 *
 * Такой отрезок не идёт ни в длину пути, ни во время движения, а
 * показывается разрывом: прямая через полгорода выглядела бы уликой, хотя
 * это туннель, подвал или севший телефон.
 *
 * ПЯТНАДЦАТЬ, А НЕ ПЯТЬ. Сначала стояло пять минут — и живая проверка на
 * настоящей базе показала «пройдено 0 метров» за день с двумя заездами и
 * дорогой между ними. Причина в том, что Telegram шлёт точки НЕ раз в
 * минуту: он экономит батарею и при ровном движении легко молчит шесть-семь
 * минут. При пороге в пять минут в длину пути не попадал вообще ни один
 * отрезок дороги.
 *
 * Из двух ошибок эта — худшая. Завышенный путь виден и обсуждаем, а
 * «прошёл ноль километров» выглядит как прогул и обвиняет человека
 * молчанием связи. Пятнадцать минут совпадают с порогом, по которому
 * сторож в боте считает трансляцию прерванной, — и это один и тот же
 * вопрос: с какого момента мы перестаём знать, где человек.
 */
export const GAP_MS = 15 * 60 * 1000;

/**
 * То же молчание, но в минутах — для людей и для ответов наружу.
 *
 * ОДНО ЧИСЛО, А НЕ ПЯТЬ. Пятнадцать минут были вписаны отдельно в сводку
 * молчания, в живой слой карты, в подпись под человеком и в сводку бота —
 * при том, что вопрос везде один: с какого момента мы перестаём знать, где
 * человек. Разъехавшись, эти копии дали бы карту, на которой точка ещё
 * «сейчас», и сообщение, в котором тот же человек уже «молчит».
 */
export const SILENT_MIN = GAP_MS / 60_000;

/**
 * Старше этого позиция не годится для УТВЕРЖДЕНИЯ о человеке.
 *
 * За полчаса уезжают через весь город: отметить по такой точке визит значит
 * подтвердить несуществующее, а предложить по ней ближайшего клиента —
 * послать не туда.
 *
 * ОТДЕЛЬНО ОТ `SILENT_MIN`, и это не дубль. Пятнадцать минут отвечают на
 * вопрос «пора ли перестать УТВЕРЖДАТЬ на карте, что мы знаем, где он»;
 * тридцать — «можно ли на этой точке основывать ДЕЙСТВИЕ». Свести их в одно
 * значит либо заставить карту врать, либо строить подсказку на позиции
 * получасовой давности.
 */
export const POSITION_FRESH_MIN = 30;
