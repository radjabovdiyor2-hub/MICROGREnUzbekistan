...
//
// Одно из двух мест в проекте, где вызывается `navigator.geolocation`
// напрямую; второе — `watch.ts` рядом, для непрерывной записи дня. Карта
// берёт позицию из контрола MapLibre, а отметке визита нужен ответ здесь
// и сейчас, до отправки запроса.
//
// РАЗДЕЛЕНЫ НАМЕРЕННО: здесь спешат и готовы уйти без ответа, там ждут и
// требуют свежести. Свести их в одну функцию значило бы дать обеим
// сторонам чужой компромисс.
//
