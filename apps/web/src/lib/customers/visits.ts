...
/**
 * Единственный признак «это визит» для ЧТЕНИЯ.
 *
 * ЗАЧЕМ ОДИН НА ВСЕХ. Признаков было два: шесть чтений отбирали по типу
 * (`interactionType IN VISIT_TYPES`), а вечерний итог объездов — по каналу
 * (`channel = 'visit'`). Совпадали они только потому, что одна и та же дверь
 * пишет оба поля. Появись третий писатель или новый тип визита — экран и
 * вечерний сигнал начали бы считать разное, и это выглядело бы ошибкой
 * экрана, а не расхождением определений.
 *
 * Отбираем по ТИПУ: префикс `visit_` различает надёжнее канала, который у
 * `interactions` имеет умолчание и может быть проставлен кем угодно.
 *
 * Объект простой, без типов Prisma: этот модуль читают и клиентские
 * компоненты (кнопки исходов), а тянуть в них клиент базы незачем.
 */
export const VISIT_WHERE = { interactionType: { in: VISIT_TYPES } };
