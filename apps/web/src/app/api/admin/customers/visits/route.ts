  ...
  try {
    ...
    const interaction = await prisma.interaction.create({
      data: {
        ...
        distanceM,
        // КТО съездил. Поле не заполнялось вовсе: отметка ложилась в
        // историю клиента без автора, и на вопрос «кто был у Плов Центра
        // во вторник» ответить было нечем — пока продавец один, это
        // незаметно, со вторым становится неразрешимым.
        //
        // Колонка называется `bot_name`, потому что офис пишет сюда имя
        // бота, сделавшего касание. Здесь касание делает человек, и его
        // имя — ровно тот же ответ на тот же вопрос.
        botName: actorOf(request).actor.slice(0, 50),
      },
      select: { id: true, createdAt: true },
    });

    // ПРИВЯЗЫВАЕМ ОТМЕТКУ К ОТКРЫТОЙ СТОЯНКЕ. Поле `interactionId` есть в
    // схеме с самого начала и не заполнялось ни разу — а без него «заехал»
    // и «отметил» сводятся по «клиент + день», то есть тем же слабым
    // соединением, из-за которого визит одного закрывал точку у всех.
    // Ломается оно на случае, который схема сама и описывает: утром не
    // застали, вечером застали.
    //
    // ЛУЧШИМИ УСИЛИЯМИ: стоянки может не быть вовсе — кнопка «Я на точке»
    // добровольная, и большинство визитов отмечаются без неё. Отказ здесь
    // НЕ должен ронять отметку визита: она и есть работа.
    //
    // `interactionId: null` в условии обязателен: поле уникально, и второй
    // визит не должен отбирать уже связанную стоянку.
    // Имя берём ТО ЖЕ, что легло в `botName` строкой выше: два способа
    // назвать одного человека разойдутся на первой правке.
    const who = actorOf(request).actor;
    if (who) {
      await prisma.trackStay.updateMany({
        where: {
          customerId,
          confirmedBy: 'manual',
          leftAt: null,
          interactionId: null,
          fieldDay: { employee: { name: who } },
        },
        data: { interactionId: interaction.id },
      });
    }

    // ЗАКРЫВАЕМ ОСТАНОВКУ СЕГОДНЯШНЕГО ПЛАНА.
    //
    // До этого у плана не было ни одного признака выполнения: он знал
    // ПОРЯДОК объезда и не знал, что из него пройдено. «Объехал 6 из 8»
    // считалось сопоставлением по паре «клиент + день» — тем же слабым
    // соединением, от которого отказались у стоянок строкой выше.
    //
    // День берём по дате визита, а не по «сегодня»: отметка из очереди
    // приезжает часами позже и должна закрыть остановку ТОГО дня, когда
    // человек там был.
    //
    // `interactionId: null` обязателен по той же причине, что и у стоянки:
    // поле уникально, и вечерний повторный заезд не должен отбирать
    // остановку у утренней отметки.
    //
    // ЛУЧШИМИ УСИЛИЯМИ: плана может не быть вовсе — к клиенту заезжают и
    // мимоходом. Отсутствие остановки не делает визит хуже.
    if (who) {
      // Полночь по МЕСТНОМУ времени: ровно так план и сохраняется
      // (`dayBounds` в visitPlanStore). Полночь по UTC не нашла бы его
      // вовсе — в Ташкенте это пять часов утра предыдущего дня.
      const planDay = new Date(visitedAt);
      planDay.setHours(0, 0, 0, 0);
      await prisma.visitPlanStop.updateMany({
        where: {
          customerId,
          interactionId: null,
          plan: { planDate: planDay, assignee: who },
        },
        data: { interactionId: interaction.id },
      });
    }
