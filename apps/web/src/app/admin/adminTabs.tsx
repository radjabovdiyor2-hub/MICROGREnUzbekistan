...
export const SELLER_TABS = [
  ...
  { id: 'my_route', ru: 'Мой рейс', uz: 'Mening reysim', icon: <Navigation size={16} /> },
  // Свой маршрут за день. Дверь `/api/admin/tracking/day` уже отдавала
  // продавцу ЕГО день — и правильно различала права, — но экрана у него
  // не было: посмотреть, где он сегодня был, человек не мог нигде.
  { id: 'my_day', ru: 'Мой день', uz: 'Mening kunim', icon: <Clock size={16} /> },
];
...
/**
 * Все существующие вкладки — ОБЕИХ ролей.
 *
 * ЗАЧЕМ ОБЪЕДИНЕНИЕ. `useAdminTab` проверяет адрес `?tab=` по этому списку и
 * молча откатывается на кассу, если вкладки в нём нет. Список собирался
 * только из групп владельца — и вкладка, которая есть ТОЛЬКО у продавца,
 * не открывалась никогда: человек нажимал «Мой день», адрес менялся, экран
 * оставался прежним. Ни ошибки, ни записи в логе.
 *
 * `my_route` работал лишь потому, что случайно объявлен в обоих списках.
 */
export const TAB_IDS = new Set<string>([
  ...ALL_TABS.map((tab) => tab.id),
  ...SELLER_TABS.map((tab) => tab.id),
]);
