...
export function AdminCustomerPurge({ onDone }: { onDone: () => void }) {
  ...
  const purge = async (target: Scope) => {
    ...
    const ok = await notify.confirm({
      ...
      detail: 'Вместе с карточкой уходят её взаимодействия и напоминания — в том числе метки о том, что этому заведению уже отправляли предложение. Вернуть их нечем.',
      confirmText: 'Удалить',
      danger: true,
    });
    if (!ok) return;
