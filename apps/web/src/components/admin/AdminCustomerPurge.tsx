...
//
// Взаимодействия удаление ПЕРЕЖИВАЮТ (связь `SetNull` в схеме), поэтому
// пакетная чистка больше не стирает пласт работы: кому звонили и кому
// отправляли предложение, остаётся в журнале.
//
...
export function AdminCustomerPurge({ onDone }: { onDone: () => void }) {
  ...
  const purge = async (target: Scope) => {
    ...
    const ok = await notify.confirm({
      ...
      detail: 'Карточки исчезнут без возврата. История касаний останется в журнале (кому звонили, кому возили пробник), а напоминания уйдут вместе с карточками.',
      confirmText: 'Удалить',
      danger: true,
    });
    if (!ok) return;
