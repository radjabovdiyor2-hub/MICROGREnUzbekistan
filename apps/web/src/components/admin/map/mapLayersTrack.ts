 ...
 *
 * Берётся из `lib/tracking/ping.ts`, а не повторяется числом: там такой
 * отрезок не идёт в длину пути, здесь — не выдаётся за известную дорогу.
 * Это один и тот же вопрос, и разойдись эти числа — карта показывала бы
 * сплошную линию там, где километры в неё не засчитаны.
 */
