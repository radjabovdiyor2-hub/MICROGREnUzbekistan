...
export async function handleClick(
  ...
  // Режим простановки пина: ЛЮБОЙ клик — новая координата.
  //
  // Здесь стояло `if (candidates.length === 0)`, то есть клик, попавший в
  // любую отрисованную точку, молча не делал ничего. Задумано это было
  // против промаха, а сломало ровно тот сценарий, ради которого режим и
  // существует: «переставить пин» нажимают у точки, которая уже стоит на
  // карте, и целятся в нескольких метрах от неё — то есть внутрь её же
  // рамки попадания (12 px пальцем). Снаружи это выглядело как «карта не
  // реагирует»: ни перестановки, ни объяснения.
  //
  // В плотном центре под рамку попадает вообще почти всё — соседние
  // заведения, цели и кластеры, — и поставить пин там было нельзя вовсе.
  //
  // Промах теперь не страшен: режим виден плашкой над картой и снимается
  // кнопкой «Отмена» или Esc, а сама перестановка обратима.
  if (now.placingId !== null) {
    now.onPlace(e.lngLat);
    return;
  }
