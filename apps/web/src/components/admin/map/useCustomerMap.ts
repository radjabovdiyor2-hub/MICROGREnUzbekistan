...
export function useCustomerMap() {
  ...
  const { data, isLoading, error, refetch, dataUpdatedAt } = useQuery<MapCollection, Error>({
    ...
    //
    // Ключ запроса включает все фильтры, поэтому любой клик по легенде,
    // городу или типу заведения заводил НОВЫЙ ключ — а у нового ключа
    // `data` равно `undefined`, и карта на время запроса становилась
    // пустой. Со стороны это выглядит так: точки исчезли. На быстрой сети
    // это мигание, на медленной — секунды пустого экрана посреди работы.
    //
    // Прежний набор на эти доли секунды честнее пустого: он показывает
    // то, что было, а не утверждает, что под фильтр не попал никто.
