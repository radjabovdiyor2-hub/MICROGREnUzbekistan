...
#: Где витрина сама зовёт `publish(...)`.
WEB_SRC = ROOT.parent / "web" / "src"


def topics_published_by_web() -> set[str]:
    """Темы, которые витрина шлёт СЕБЕ САМА.

    Не всякую тему присылает офис. `field` публикует приёмник крошек трека
    (`api/admin/tracking/ping`): человек в поле идёт мимо офиса, крошка
    ложится прямо в витрину, и она же оповещает свои экраны.

    Для сверки ниже важно одно: у темы должен быть ХОТЬ КАКОЙ-ТО отправитель.
    Тема без отправителя — это экран, который ждёт сигнала, которого не
    будет; а вот чей это отправитель — офис или сама витрина — уже не всё
    равно только для настройки, но не для работоспособности.
    """
    found: set[str] = set()
    ...
    for path in WEB_SRC.rglob("*.ts"):
        ...
        for call in re.findall(r"(?<![A-Za-z])publish\(([^)]*)\)", text):
            found.update(re.findall(r"'([a-z_]+)'", call))
    return found


def check_realtime_topics_match() -> None:
    """У каждой темы живых обновлений есть отправитель, и он известен обоим.
    ...
    ОБРАТНАЯ СТОРОНА МЯГЧЕ. Тема, которой нет у офиса, — беда только если её
    не шлёт никто. Витрина публикует часть тем сама (см.
    `topics_published_by_web`), и требовать от офиса уметь слать `field`
    значило бы завести в нём возможность, которой он не пользуется.
    """
    ...
    # Тему, которую витрина публикует сама, офису знать необязательно.
    orphans = only_web - topics_published_by_web()
    if orphans:
        problems.append(
            f"тему не шлёт никто — ни офис, ни витрина: "
            f"{', '.join(sorted(orphans))} — экран будет ждать сигнала, которого нет"
        )
