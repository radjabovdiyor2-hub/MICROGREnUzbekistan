...
async def dashboard(cb: CallbackQuery):
    ...
    await cb.message.edit_text(
        ...
        f"📦 Заказов: {d.cnt}\n"
        f"🧾 Средний чек: {format_price(avg)}", reply_markup=back_kb())
    await cb.answer()
...
async def top_products(cb: CallbackQuery):
    async with get_session_ctx() as session:
        ...
        products = r.fetchall()
    if not products:
        await cb.message.edit_text("🏆 Пока нет данных о продажах.", reply_markup=back_kb())
    else:
        medals = ["🥇","🥈","🥉","4.","5."]
...
async def customers(cb: CallbackQuery):
    ...
    await cb.message.edit_text(
        ...
        f"⭐ VIP: {c.vip}\n"
        f"✅ Активные: {c.active}\n"
        f"❌ Отток: {c.churned}", reply_markup=back_kb())
    await cb.answer()
...
async def abc_analysis(cb: CallbackQuery):
    ...
    if not all_prods:
        await cb.message.edit_text("📋 Нет данных для ABC-анализа.", reply_markup=back_kb())
