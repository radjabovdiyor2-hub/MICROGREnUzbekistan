...
# ── Журнальный конвейер УДАЛЁН ───────────────────────────────────────────
#
# FRESH WEEKLY делается вручную: вёрстка, сверки, публикация скриптом
# scripts/publish-magazine.mjs, карточка номера в админке витрины. Автоматика
# тут не помогала, а мешала — `finalize` каждый четверг публиковал последний
# неопубликованный выпуск, в том числе тот, который владелец в этот момент
# готовил руками.
#
# Здесь стояли `run_magazine_pipeline` и три обёртки над веб-кронами
# (`prepare`, `finalize`, `print-run`). Расписание им отключили ещё раньше, а
# сами роуты удалены вместе с конвейером — функции звали несуществующие
# адреса и подпроцесс `scripts/generate-magazine-pdf.js`, которого в
# репозитории нет.
#
# Счёт за тираж теперь выставляется по вышедшему номеру на вкладке «Тираж и
# реклама» (POST /api/admin/magazine/print-run).
