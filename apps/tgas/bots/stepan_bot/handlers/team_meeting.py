...
from shared.utils import contains_any, format_price, collapsible
...
DEPARTMENTS = {
    ...
    "pm": {
        ...
        "role": (
            "Ты — Степан, Генеральный Управляющий и Операционный директор (COO). Твоя зона: "
            "себестоимость, свежесть и скоропорт, логистика, дедлайны."
        ),
    },
    # У R&D и DevOps нет Telegram-интерфейса (bot_registry: telegram=False) —
    # своего «голоса» в чате у них быть не может. Но их экспертиза на совещании
    # нужна: спрос на новые позиции и работоспособность сервисов влияют на
    # любое операционное решение. Поэтому они говорят через бота руководителя.
    # Заводить им отдельные токены ради реплики в чате не нужно.
    "rnd": {
        "name": "R&D",
        "emoji": "🔬",
        "token": "stepan_bot_token",
        "role": (
            "Ты — аналитик R&D. Твоя зона: спрос на новые позиции, тренды "
            "аудитории и расширение ассортимента."
        ),
    },
    "devops": {
        "name": "DevOps / IT",
...
def is_meeting_request(low_text: str) -> bool:
    ...
    «Заседания», «планёрки» и «летучки» здесь долго не было, хотя механизм
    совещания готов и работает: 10.08.2026 просьба «поставь этот вопрос на
    общее заседание» до него не дошла и уехала в отдел контента.
    """
    if contains_any(low_text, MEETING_TRIGGERS):
        return True
    # «почему нет заказов», «продажи упали» и т.п.
    if "почему" in low_text and contains_any(low_text, _WHY_TOPICS):
        return True
    return False
...
_EXEC_TRIGGERS = [
    # ⚠️ Корни со звёздочкой, а не точные формы: «выполни» ⊄ «выполняй» —
    # из-за этого «Выполняй» раньше не распознавалось вовсе и уходило в общий
    # AI, который выдумывал задачу (вплоть до реальной публикации).
    # Звёздочка = совпадение по НАЧАЛУ слова, поэтому «делай*» покрывает
    # «делай/делайте», но не находится внутри «переделай».
    "делай*",
    "сделай*",
    "выполн*",
    "исполн*",
...
def is_execution_command(low_text: str) -> bool:
    ...
    if len(t) > 60:
        return False
    return contains_any(t, _EXEC_TRIGGERS)
