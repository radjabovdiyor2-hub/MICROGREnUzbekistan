...
async def create_task(cb: CallbackQuery, state: FSMContext) -> None:
...
async def task_title(msg: Message, state: FSMContext) -> None:
...
async def task_priority(cb: CallbackQuery, state: FSMContext) -> None:
    pri = cb.data.split(":")[-1]
...
async def task_desc(msg: Message, state: FSMContext) -> None:
...
async def confirm_task(cb: CallbackQuery, state: FSMContext) -> None:
    d = await state.get_data()
...
async def cancel_task(cb: CallbackQuery, state: FSMContext) -> None:
    d = await state.get_data()
    await state.set_state(None)
    await cb.message.edit_text(
...
async def my_tasks(cb: CallbackQuery, state: FSMContext) -> None:
