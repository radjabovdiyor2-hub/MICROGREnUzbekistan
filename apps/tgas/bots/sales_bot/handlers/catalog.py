...
CATEGORY_MAP = {
    "cat:microgreens": "microgreens", "cat:baby-leaf": "baby-leaf",
    "cat:salads": "salads", "cat:flowers": "flowers",
    "cat:seeds": "seeds", "cat:substrate": "substrate",
    "cat:equipment": "equipment", "cat:sets": "sets",
}
...
async def show_products(cb: CallbackQuery, state: FSMContext):
    ...
    async with get_session_ctx() as session:
        result = await session.execute(
            text("SELECT id, name_ru, name_uz, price, unit, description_ru, description_uz "
    ...
    if not products:
        await cb.message.edit_text(
            "😔 В этой категории пока нет товаров." if lang == "ru" else "😔 Bu kategoriyada hozircha mahsulot yo'q.",
    ...
    b.row(*[
        InlineKeyboardBuilder().button(
            ...
            callback_data="menu:cart").buttons[0],
        InlineKeyboardBuilder().button(
            text="⬅️ Назад" if lang == "ru" else "⬅️ Orqaga", 
            callback_data="menu:catalog").buttons[0]
    ])
...
async def add_to_cart(cb: CallbackQuery, state: FSMContext):
    ...
    async with get_session_ctx() as session:
        result = await session.execute(
            text("SELECT id, name_ru, name_uz, price, unit FROM products WHERE id = :pid"),
            {"pid": int(product_id)})
    ...
    name = product.name_uz if lang == "uz" else product.name_ru
    await cb.answer(f"✅ {name} добавлен в корзину!" if lang == "ru" else f"✅ {name} savatga qo'shildi!")
...
async def show_cart(cb: CallbackQuery, state: FSMContext):
    ...
    lines.append(f"\n💰 <b>Итого: {format_price(total)}</b>" if lang == "ru" else f"\n💰 <b>Jami: {format_price(total)}</b>")
