...
async def process_company(message: Message, state: FSMContext):
    ...
    await message.answer(
        "📦 Какой примерный объём поставок вас интересует (кг/месяц)?" if lang == "ru"
...
async def process_volume(message: Message, state: FSMContext):
    ...
    await message.answer(
        "📞 Ваш контактный телефон:" if lang == "ru"
        else "📞 Aloqa telefoningiz:")
...
async def process_contact(message: Message, state: FSMContext):
    ...
    async with get_session_ctx() as session:
        await session.execute(
            text("INSERT INTO interactions (customer_id, channel, interaction_type, bot_name, summary, created_at) "
