...
async def start_ai_chat(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(
        "💬 Задайте любой вопрос о нашей продукции!" if lang == "ru"
...
async def show_orders(cb: CallbackQuery, state: FSMContext):
    ...
    async with get_session_ctx() as session:
        res = await session.execute(
            sqt("SELECT order_number, total_amount, status, created_at FROM orders "
    ...
    if not orders:
        await cb.message.edit_text(
            "📦 У вас пока нет заказов" if lang == "ru" else "📦 Sizda hali buyurtmalar yo'q",
            reply_markup=main_menu_kb(lang))
    else:
        from shared.utils import format_price
        lines = ["📦 <b>Ваши заказы:</b>\n" if lang == "ru" else "📦 <b>Buyurtmalaringiz:</b>\n"]
        for o in orders:
            lines.append(f"#{o.order_number} — {format_price(o.total_amount)} ({o.status})")
...
async def ai_vision_recipe(message: Message, state: FSMContext):
    ...
    try:
        ...
        await msg.edit_text("Извините, не удалось проанализировать фотографию. Попробуйте еще раз позже.")
