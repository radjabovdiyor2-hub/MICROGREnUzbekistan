...
async def process_payment(cb: CallbackQuery, bot: Bot):
    ...
    # Формируем счет
    prices = [LabeledPrice(label=f"Заказ {order_number}", amount=int(total_amount * 100))] # В тийинах (копейках)
    
    # Если токен не настроен
    provider_token = settings.payment_provider_token if hasattr(settings, 'payment_provider_token') else "TEST_TOKEN"
    ...
    if payload.startswith("mg_order_"):
        ...
        async with get_session_ctx() as session:
            await session.execute(
                text("UPDATE orders SET payment_status = 'paid', payment_method = 'online' WHERE id = :oid"),
                {"oid": order_id}
            )
            ...
            await event_bus.publish("PAYMENT_RECEIVED", {
                "order_id": order_id,
