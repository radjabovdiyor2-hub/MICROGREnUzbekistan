    ...
    try:
        ...
        if "кп" in title or "коммерческое" in title or "кп" in desc or "коммерческ" in desc:
            ...
            async with get_session_ctx() as session:
                ...
                res = await session.execute(text("SELECT COALESCE(MAX(CAST(SUBSTRING(order_number FROM 4) AS INTEGER)), 0) + 1 FROM orders"))
                next_num = res.scalar() or 1
                order_number = f"MG-{str(next_num).zfill(6)}"
                
                # Создаем заказ (customer_id = NULL так как клиент с IG пока не привязан к TG)
                await session.execute(text(
