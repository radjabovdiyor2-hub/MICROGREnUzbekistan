"""Sales Bot — отдел продаж офиса.

ЧТО ЭТОТ БОТ ДЕЛАЕТ, А ЧЕГО НЕТ

Он работает на СТОРОНЕ КОМПАНИИ: принимает задачи отдела продаж, ведёт B2B
(заявки ресторанов, коммерческие предложения), отвечает в рабочих группах
своими инструментами и показывает клиенту его заказы из CRM.

Клиентской витрины здесь больше нет — ни каталога, ни корзины, ни чекаута.
Магазин у покупателя ОДИН: витринный бот apps/bot.

ПОЧЕМУ ТАК. Оба бота вели человека через каталог → корзину → чекаут, но по
разным данным: корзина в cart_storage против FSM-состояний, каталог по HTTP
против прямого SQL, покупатель в витринных `users` против CRM `customers`,
доставка из /api/config против settings_store. Клиент, писавший одному боту,
для второго был новым человеком: другие баллы, другая история заказов, свой
способ посчитать доставку. Две реализации одного и того же расходились
молча, и заметить это можно было только по жалобе.

Кнопка «Заказать» ведёт в витринный бот, чтобы человек не упирался в тупик;
адрес берётся из настроек витрины, а не вписан строкой.
"""
...
TRACK_HINT = (
    LIVE_HINT + "\n\n"
    "С приложением Microgreen смена пишется сама — и с погашенным экраном.\n"
    "Без Telegram и приложения: откройте админку и нажмите «Начал смену» "
    "(браузер пишет, пока экран включён)."
)

# ЧЕМ ШЁЛ ТРЕК — ТАКОЙ И СОВЕТ.
#
# Один текст на всех велел человеку с приложением «включить трансляцию в
# Telegram», которой он не пользуется, — и смена продолжала молчать. У трёх
# способов записи разные причины молчания и разное лечение: трансляция
# гаснет через восемь часов сама, вкладка засыпает с погашенным экраном, а
# приложение усыпляет экономия батареи — на Xiaomi и Samsung по умолчанию.
# Источник последней крошки отдаёт витрина (`lastSource`).
APP_SILENT_HINT = (
    "Откройте приложение Microgreen и проверьте, что в шторке висит "
    "уведомление «Смена идёт».\n\n"
    "Не смахивайте приложение из списка открытых — так запись "
    "останавливается.\n\n"
    "Трек рвётся снова — настройки телефона: Приложения → Microgreen → "
    "Батарея → «Без ограничений». Иначе телефон усыпляет запись."
)
PWA_SILENT_HINT = (
    "Браузер пишет маршрут, только пока экран включён и вкладка открыта.\n\n"
    + LIVE_HINT
)
# То же молчание словами для владельца — теми же различениями.
_SILENT_WHAT = {
    "app": "приложение перестало присылать точки",
    "pwa": "вкладка браузера перестала присылать точки",
}


def silent_text(person: dict) -> str:
    """Текст сотруднику о молчании — по источнику последней крошки.

    Неизвестный источник читается как трансляция: так было до появления
    `lastSource`, и витрина старой версии, не отдающая поле, не должна
    оставить человека вовсе без совета.
    """
    minutes = person.get("silentMin")
    source = person.get("lastSource")
    if source == "app":
        return (
            "🛰 <b>Приложение перестало присылать точки</b>\n\n"
            f"Смена идёт, но точек нет уже {minutes} мин.\n\n{APP_SILENT_HINT}"
        )
    if source == "pwa":
        return (
            "🛰 <b>Вкладка перестала присылать точки</b>\n\n"
            f"Смена идёт, но точек нет уже {minutes} мин.\n\n{PWA_SILENT_HINT}"
        )
    return (
...
def track_nudge(person: dict) -> Optional[tuple[str, str]]:
    ...
    if state == "silent":
        # Повод один на все источники — по нему считается повтор. Разные
        # только слова: совет обязан подходить тому, чем человек пишет.
        return "silent", silent_text(person)
...
def owner_alert(person: dict, reason: str, delivered: bool) -> tuple[str, str]:
    ...
    if reason == "never":
        ...
        # Что замолчало — теми же различениями, что в тексте сотруднику:
        # «трансляция прервалась» про человека с приложением подсказала бы
        # владельцу не тот вопрос.
        gone = _SILENT_WHAT.get(str(person.get("lastSource") or ""), "трансляция прервалась")
        what = f"{gone}, точек нет {person.get('silentMin')} мин"
    elif reason == "plan":
        assigned = int(person.get("planTotal") or 0) + int(person.get("routeTotal") or 0)
