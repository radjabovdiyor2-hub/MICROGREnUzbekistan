"""Finance Bot — Start + All Handlers"""
...
from bots.finance_bot.keyboards.inline import fin_menu_kb, back_kb, expense_categories_kb, income_categories_kb
...
async def cmd_start(msg: Message):
    await msg.answer("💰 <b>Finance Bot</b>\n\nУчёт финансов, P&L и зарплаты.", reply_markup=fin_menu_kb())

@router.callback_query(F.data == "fin:menu")
async def menu(cb: CallbackQuery): await cb.message.edit_text("💰 Финансы:", reply_markup=fin_menu_kb()); await cb.answer()
...
async def expense(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text("💸 Выберите категорию расхода:", reply_markup=expense_categories_kb()); await cb.answer()
...
async def exp_category(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text(f"💸 Категория: {cat.capitalize()}\nВведите сумму расхода (в сумах):")
    await cb.answer()
...
async def exp_desc(msg: Message, state: FSMContext):
    ...
    if d["amount"] >= 1000000:
        ...
        await event_bus.publish(Events.LARGE_EXPENSE_ALERT, {
            "amount": d["amount"], "description": msg.text
        }, source_bot="finance_bot")
    await state.clear()
    await msg.answer(f"✅ Расход {format_price(d['amount'])} записан!", reply_markup=fin_menu_kb())
...
async def income(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text("💰 Выберите категорию дохода:", reply_markup=income_categories_kb()); await cb.answer()
...
async def inc_desc(msg: Message, state: FSMContext):
    ...
    await state.clear()
    await msg.answer(f"✅ Доход {format_price(d['amount'])} записан!", reply_markup=fin_menu_kb())
...
async def pnl(cb: CallbackQuery):
    ...
    await cb.message.edit_text(
        ...
        f"{emoji} <b>Прибыль: {format_price(profit)}</b>", reply_markup=back_kb())
    await cb.answer()
...
async def balance(cb: CallbackQuery):
    async with get_session_ctx() as session:
        ...
        bal = r.scalar() or 0
    await cb.message.edit_text(f"🏦 <b>Баланс:</b> {format_price(bal)}", reply_markup=back_kb())
    await cb.answer()
...
async def debts(cb: CallbackQuery):
    async with get_session_ctx() as session:
        ...
        debts = r.fetchall()
    if not debts:
        await cb.message.edit_text("💳 Нет неоплаченных заказов!", reply_markup=back_kb())
    else:
        lines = ["💳 <b>Дебиторка</b>\n━━━━━━━━━━━━━━━━━━"]
        for d in debts: lines.append(f"🔴 #{d.order_number} — {d.name} — {format_price(d.total_amount)}")
...
async def salary(cb: CallbackQuery):
    async with get_session_ctx() as session:
        r = await session.execute(text("SELECT name, role, salary FROM employees WHERE status='active' AND salary > 0"))
        emps = r.fetchall()
    if not emps:
        await cb.message.edit_text("💼 Нет сотрудников с зарплатой.", reply_markup=back_kb())
    else:
        total = sum(e.salary for e in emps)
        lines = ["💼 <b>Зарплаты</b>\n━━━━━━━━━━━━━━━━━━"]
        for e in emps: lines.append(f"👤 {e.name} ({e.role}) — {format_price(e.salary)}")
        lines.append(f"\n💰 <b>Итого ФОТ: {format_price(total)}</b>")
        await cb.message.edit_text("\n".join(lines), reply_markup=back_kb())
    await cb.answer()

@router.callback_query(F.data == "fin:ai")
async def start_ai(cb: CallbackQuery): await cb.message.edit_text("🤖 Задайте финансовый вопрос:", reply_markup=back_kb()); await cb.answer()
