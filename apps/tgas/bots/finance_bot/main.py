        ...
        try:
            ...
            if rows:
                ...
                for row in rows[:15]:
                    ...
                    days = int(row[3])
                    total += amount
                    lines.append(f"• #MG-{oid:04d} — {'{:,.0f}'.format(amount)} сум ({days} дн.)")
        ...
        try:
            ...
            if rows:
                ...
                for row in rows:
                    ...
                    amount = row[2]
                    desc = (row[3] or "")[:60]
                    lines.append(f"• #{fid} [{category}]: {'{:,.0f}'.format(amount)} сум — {desc}")
                await bot.send_message(admin_id, "\n".join(lines), parse_mode="HTML")
        ...
        try:
            ...
            if categories:
                ...
                for cat in categories:
                    report_text += f"  {cat[0]}/{cat[1] or '—'}: {'{:,.0f}'.format(cat[2])} сум\n"
            ...
            logger.info("monthly_pnl: income=%s expense=%s profit=%s", income, expense, profit)
        finally:
            await bot.session.close()
    except Exception as e:
...
async def salary_reminder():
    ...
    try:
        ...
        try:
            ...
            logger.info("salary_reminder: %d сотрудников, фонд %s", count, f"{total:,.0f}")
        finally:
            await bot.session.close()
    except Exception as e:
...
async def handle_payment_received(payload: dict):
    ...
    import logging
    logging.info(f"FINANCE BOT: оплата заказа {order_id}: {amount} UZS (доход уже учтён при создании заказа)")
    ...
    try:
        ...
        admin_id = settings.admin_telegram_ids[0]
        try:
            await bot.send_message(admin_id, f"✅ <b>Поступление оплаты!</b>\n\nСумма: {amount:,.0f} UZS\nЗаказ ID: {order_id}")
        except Exception:
            pass
        finally:
            await bot.session.close()
...
async def main():
    ...
    asyncio.create_task(bus_listen("finance_bot", {
