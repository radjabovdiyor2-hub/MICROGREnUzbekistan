...
async def production(cb: CallbackQuery):
    await cb.message.edit_text(
        ...
        "📋 <b>Посев:</b>\n• Руккола — 5 лотков\n• Базилик — 3 лотка\n\n"
        "🌿 <b>Сбор урожая:</b>\n• Шпинат — ~3 кг\n• Горох — ~2 кг\n\n"
        "📦 <b>Упаковка:</b>\n• Заказы на сегодня — формируются", reply_markup=back_kb())
    await cb.answer()
...
async def logistics(cb: CallbackQuery):
    async with get_session_ctx() as session:
        ...
        orders = res.fetchall()
    if not orders:
        await cb.message.edit_text("🚚 Нет активных доставок на сегодня.", reply_markup=back_kb())
    else:
        lines = [f"🚚 <b>Доставки: {len(orders)}</b>\n━━━━━━━━━━━━━━━━━━"]
        for o in orders:
            lines.append(f"📍 #{o.order_number} — {o.delivery_address or 'Адрес не указан'} ({o.status})")
...
async def overview(cb: CallbackQuery):
    async with get_session_ctx() as session:
        tasks_r = await session.execute(text("SELECT COUNT(*) FILTER (WHERE status='todo') as todo, COUNT(*) FILTER (WHERE status='in_progress') as wip, COUNT(*) FILTER (WHERE status='done') as done FROM tasks"))
        t = tasks_r.fetchone()
        orders_r = await session.execute(text("SELECT COUNT(*) FROM orders WHERE status NOT IN ('delivered','cancelled')"))
        active_orders = orders_r.scalar() or 0
    await cb.message.edit_text(
        f"📊 <b>Обзор проекта</b>\n━━━━━━━━━━━━━━━━━━\n"
        f"✅ Задачи: ⬜{t.todo} | 🔄{t.wip} | ✅{t.done}\n"
        f"📦 Активные заказы: {active_orders}\n"
        f"👥 Команда: на связи", reply_markup=back_kb())
    await cb.answer()
