...
from shared.utils import simulate_typing, format_price
import json, logging
...
async def dispatch_task(msg: Message):
    """Получает текст задачи, классифицирует через AI и создаёт задачу."""
    # Если сообщение адресовано Степану, PM бот не должен его перехватывать
    if msg.text.lower().startswith(("@степан", "степан", "@stepan")):
        return
    ...
    try:
        response = await ai.chat_completion(CLASSIFIER_PROMPT, msg.text)
        ...
        await msg.answer(
            ...
            "💡 <b>Пример:</b>\n"
            "• Подготовить 50 лотков руколы к пятнице\n"
            "• Написать пост в Instagram про новый урожай\n"
            "• Доставить заказ #MG-000015 клиенту на Регистан"
        )
        return

    department = task_data.get("department", "production")
    priority = task_data.get("priority", "medium")
    assignee = task_data.get("assignee", "Менеджер")
    title = task_data.get("title", msg.text[:100])
    steps = task_data.get("steps", [])
    deadline_days = task_data.get("deadline_days", 3)
    verification = task_data.get("verification", "Проверить результат")
    ...
    async with get_session_ctx() as session:
        result = await session.execute(text(
            ...
            "VALUES (:title, :assignee, :dept, 'todo', :priority, :desc, "
            "CURRENT_DATE + CAST(:days AS integer), NOW()) RETURNING id"),
            {
                "title": title, "assignee": assignee, "dept": department,
                "priority": priority, "days": deadline_days,
                "desc": json.dumps({
    ...
    # Публикуем событие
    await event_bus.publish(Events.TASK_CREATED, {
        "task_id": task_id, "title": title, "department": department,
        "assignee": assignee, "priority": priority, "steps": steps
    }, source_bot="pm_bot")

    # Формируем красивый ответ
    emoji = DEPT_EMOJI.get(department, "📋")
    dept_name = DEPT_NAME.get(department, department)
    priority_emoji = {"urgent": "🔴", "high": "🟠", "medium": "🟡", "low": "🟢"}.get(priority, "⚪")

    steps_text = "\n".join(f"  {i+1}. {s}" for i, s in enumerate(steps))

    response_text = (
        f"{emoji} <b>Задача #{task_id} создана</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"📌 <b>{title}</b>\n\n"
        f"🏢 Отдел: {dept_name}\n"
        f"👤 Ответственный: {assignee}\n"
        f"{priority_emoji} Приоритет: {priority}\n"
        f"📅 Дедлайн: {deadline_days} дн.\n\n"
        f"📋 <b>Шаги:</b>\n{steps_text}\n\n"
        f"🔍 <b>Проверка:</b> {verification}"
    )

    await msg.answer(response_text, reply_markup=task_actions_kb(task_id))
