...
VACANCIES = [
    ("🌱 Фермер микрозелени", "Посев, полив, сбор урожая. Опыт не требуется.", "15 000 - 20 000 сум/день"),
    ("🚚 Курьер", "Доставка заказов по Самарканду. Авто/мото.", "20 000 - 30 000 сум/день"),
...
async def cmd_start(msg: Message):
    await msg.answer("👥 <b>HR Bot</b>\n\nВакансии, заявки и управление персоналом.", reply_markup=hr_menu_kb())

@router.callback_query(F.data == "hr:menu")
async def menu(cb: CallbackQuery): await cb.message.edit_text("👥 HR:", reply_markup=hr_menu_kb()); await cb.answer()
...
async def app_position(msg: Message, state: FSMContext):
    ...
    async with get_session_ctx() as session:
        await session.execute(text(
            ...
            {"tid": msg.from_user.id, "s": f"HR Заявка: {d['name']} | Тел: {d['phone']} | Должность: {msg.text}"})
    await state.clear()
    # 🔗 EventBus: уведомляем PM бота
    from shared.event_bus import event_bus, Events
    await event_bus.publish(Events.APPLICATION_RECEIVED, {
        "name": d['name'], "phone": d['phone'], "position": msg.text
    }, source_bot="hr_bot")
    await msg.answer(f"✅ <b>Заявка принята!</b>\n\n👤 {d['name']}\n📞 {d['phone']}\n💼 {msg.text}\n\nМы свяжемся с вами!", reply_markup=hr_menu_kb())

@router.callback_query(F.data == "hr:shifts")
async def shifts(cb: CallbackQuery):
    async with get_session_ctx() as session:
        r = await session.execute(text("SELECT name, role, status FROM employees WHERE status='active' LIMIT 10"))
        emps = r.fetchall()
    if not emps:
        await cb.message.edit_text("⏰ Нет активных сотрудников.", reply_markup=back_kb())
    else:
        lines = ["⏰ <b>Табель сотрудников</b>\n━━━━━━━━━━━━━━━━━━"]
        for e in emps: lines.append(f"✅ {e.name} — {e.role}")
...
async def leave(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text("🏖 Выберите тип отсутствия:", reply_markup=leave_type_kb()); await cb.answer()
...
async def leave_reason(msg: Message, state: FSMContext):
    ...
    async with get_session_ctx() as session:
        await session.execute(text(
            ...
            {"tid": msg.from_user.id, "s": f"Отпуск/Больничный: {d['leave_type']} | С: {d['start_date']} По: {d['end_date']} | Причина: {msg.text}"})
    await state.clear()
    
    # 🔗 EventBus: уведомляем PM бота (или HR)
    from shared.event_bus import event_bus, Events
    # Можно отправить просто как NEW_MESSAGE, если нет специального ивента
    await event_bus.publish(Events.NEW_MESSAGE, {
...
async def training(cb: CallbackQuery):
    await cb.message.edit_text(
        ...
        "5. 💬 Работа с клиентами\n6. 🍽 Микрозелень в HoReCa", reply_markup=back_kb())
    await cb.answer()

@router.callback_query(F.data == "hr:ai")
async def start_ai(cb: CallbackQuery): await cb.message.edit_text("🤖 Задайте HR вопрос:", reply_markup=back_kb()); await cb.answer()
