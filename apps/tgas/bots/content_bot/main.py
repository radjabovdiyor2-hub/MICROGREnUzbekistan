...
async def product_description_audit():
    ...
    try:
        ...
        lines = [
            f"📝 <b>Аудит описаний продуктов</b>\n",
            f"━━━━━━━━━━━━━━━━━━━━━━\n",
        ...
        lines.append(f"\n━━━━━━━━━━━━━━━━━━━━━━")
        lines.append(f"💡 <i>Создайте описания для повышения продаж!</i>")
        lines.append(f"🎨 <i>Content Bot — еженедельный аудит</i>")
...
async def afternoon_post():
    ...
    try:
        ...
        prompt = (
            f"Создай дневной пост (14:00) для Microgreen Uzbekistan. "
            f"Формат: UGC (пользовательский контент). Напиши реалистичный, эстетичный отзыв от лица выдуманного шеф-повара известного ресторана в Самарканде или от довольного клиента о том, как наша микрозелень преобразила их блюдо.\n"
            f"Пост должен быть в кавычках (цитата), с эмодзи. Без упоминания того, что это сгенерировано ИИ."
        )
        post_text = await ai.chat_completion(
            "Ты копирайтер.",
            prompt
...
async def handle_task_created(payload: dict):
    ...
    try:
        ...
        logging.info(f"CONTENT_BOT Generating AI answer...")
        ...
        image_url = None
        if needs_photo:
            logging.info(f"CONTENT_BOT Generating DALL-E image...")
            headline = await ai.chat_completion(
        ...
        logging.info(f"CONTENT_BOT successfully handled task.")
