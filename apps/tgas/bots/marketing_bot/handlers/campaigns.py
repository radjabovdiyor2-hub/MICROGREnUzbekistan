...
from bots.marketing_bot.keyboards.inline import back_kb, segments_kb, confirm_kb, mkt_menu_kb
router = Router()
ai = AIEngine()
...
async def create_campaign(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text("📢 <b>Создание рассылки</b>\n\nВыберите сегмент:", reply_markup=segments_kb())
    await cb.answer()
...
async def show_segments(cb: CallbackQuery):
    ...
    await cb.message.edit_text(
        ...
        f"🏢 B2B: {c.b2b}\n🛒 B2C: {c.b2c}\n"
        f"⭐ VIP: {c.vip}\n✅ Активные: {c.active}\n"
        f"❌ Отток: {c.churned}\n📊 Всего: {c.total}", reply_markup=back_kb())
    await cb.answer()
...
async def show_promo(cb: CallbackQuery):
    await cb.message.edit_text(
        ...
        "🌐 microgreenuzbekistan.com", reply_markup=back_kb())
    await cb.answer()
...
async def mkt_analytics(cb: CallbackQuery):
    ...
    await cb.message.edit_text(
        ...
        f"📨 Взаимодействий: {d.cnt}\n📦 Из них заказы: {d.orders}", reply_markup=back_kb())
    await cb.answer()
...
async def competitors(cb: CallbackQuery):
    ...
    await cb.message.edit_text(f"🔍 <b>Конкурентный анализ:</b>\n\n{resp}", reply_markup=back_kb())
    await cb.answer()
...
async def ai_mkt(msg: Message):
    ...
    from shared.prompts import TEAM_CONTEXT
    resp = await ai.chat_completion(f"{TEAM_CONTEXT}\n\nТы маркетолог Microgreen Uzbekistan. Помогай пользователю с маркетинговыми задачами. Если задача выходит за рамки маркетинга, отправь его к Степану или профильному боту.", msg.text)
