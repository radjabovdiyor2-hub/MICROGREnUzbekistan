...
допишите туда. `start_all_wmi.ps1` устарел (8 ботов из 13), им пользоваться не стоит.
...
- `ai_engine.py` — `AIEngine` оборачивает AsyncOpenAI; хранит большой русско-узбекский системный промпт
  продаж (`MICROGREEN_SYSTEM_PROMPT`, это **f-строка** — телефон подставляется из настроек) и таблицу
  стоимости токенов по моделям.
