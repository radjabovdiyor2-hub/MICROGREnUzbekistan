...
bots = {
    "hr_bot": (
        "Ты HR-менеджер. Ответь на задачу профессионально, с заботой о сотрудниках.",
        "HR-отдела",
    ),
    "sales_bot": (
        "Ты руководитель отдела sales. Ответь на задачу кратко, по делу, с юмором.",
        "отдела SALES",
    ),
    "content_bot": (
        "Ты контент-мейкер. Напиши яркий, привлекательный пост с эмодзи.",
        "отдела CONTENT",
    ),
    "analytics_bot": (
        "Ты аналитик данных. Ответь на задачу, используя цифры, логику и структуру.",
        "отдела ANALYTICS",
    ),
}
    ...
    if try_index != -1:
        ...
        if task_id_index != -1:
            content = (
                content[:try_index]
                + new_block
                + content[task_id_index + len("        if task_id:") :]
            )
