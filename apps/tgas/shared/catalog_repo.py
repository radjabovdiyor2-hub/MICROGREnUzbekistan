...
async def resolve(query: Optional[str]) -> Dict[str, Any]:
    ...
    (за ЛОТОК), и в бейби-листе (за 100 г). Цена у тёзок расходится в обе
    стороны и по категориям не постоянна — например, Базилик стоит 15 000
    в микрозелени и 35 000 в бейби-листе, а Мизуна зелёная 15 000 против
    25 000. Конкретные суммы смотрите в каталоге, не здесь. Здесь стояло
...
async def price_list(category_slug: Optional[str] = None) -> str:
    ...
    # Заголовок — человеческое имя категории, а не слаг.
    #
    # Здесь стояло `[{slug}]`, то есть `[baby-leaf]`. Слаг ничего не значит для
    # того, кто читает прайс: в группе «Продажа» этот блок был показан как
    # «прайс-лист на микрозелень», хотя бейби-лист продаётся за 100 г по
    # 25 000–40 000, а микрозелень — за ЛОТОК по 15 000–20 000. Цены при этом
    # были верные, ошибся только заголовок — и он же решает, что человек поймёт.
    titles = {c["slug"]: c["title"] for c in await categories()}

    lines: List[str] = []
    for slug, group in grouped.items():
        lines.append(f"[{titles.get(slug) or slug}]")
        for item in group:
