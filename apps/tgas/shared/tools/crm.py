...
register(
    Tool(
        ...
        description=(
            ...
            "записать человека или заведение и НЕ называют товар: «Клиент Иван, "
            "номер такой-то, зарегистрируй» — это сюда, а не в register_sale. Сам "
            "проверит, нет ли такого — и если похожий найдётся, вернёт его "
            "вместо второй карточки. Вместе с продажей вызывать НЕ нужно: "
            "register_sale заводит клиента сам."
        ),
        run=add_customer,
        departments=DEPTS,
        params={
