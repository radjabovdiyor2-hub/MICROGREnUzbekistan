...
async def record_ai_usage(
    ...
    try:
        ...
        # `warning`, а не `debug`: боты подняты с `level=logging.INFO`, и
        # debug-строка не попадает в лог вообще — то есть это было молчание.
        #
        # Молчать тут нельзя дважды. Во-первых, это ДЕНЬГИ: расход на модель
        # виден владельцу только по этой таблице. Во-вторых, по ней же
        # считается дневной и месячный бюджет — не записали расход, и
        # ограничитель не сработает никогда, потому что потрачено «ноль».
        logger.warning("Расход на ИИ не записан (%s): %s", bot_name, e)
