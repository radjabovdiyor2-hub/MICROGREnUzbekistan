...
def get_daily_morning_format(d: Optional[date] = None) -> dict:
    """
    Формат утреннего сторис на день.

    ПОЧЕМУ НЕ ПРОСТО `tm_yday % len`. Форматов ровно семь, и в неделе семь
    дней: остаток от деления дня года на семь МЕНЯЕТСЯ ВМЕСТЕ С ДНЁМ НЕДЕЛИ
    и потому от него не отличается. Каждый понедельник выпадал «факт»,
    каждый вторник — «вопрос», и так круглый год. Подписчик видит ленту, где
    неделя в неделю повторяется один и тот же макет, — а формат задаёт и
    фото, и оверлей, и триггер вовлечения, то есть весь внешний вид поста.
    Это и есть та однообразность, которую видно невооружённым глазом.

    Остальные списки ротации от этого свободны случайно: в них 11, 12, 15 и
    33 элемента — ни одно не кратно семи.

    ЧТО ДЕЛАЕТ СДВИГ НА НОМЕР НЕДЕЛИ. В пределах одной недели дни по-прежнему
    получают разные форматы. Но на фиксированном дне недели индекс каждую
    неделю сдвигается на единицу, и за семь недель этот день проходит все
    семь форматов. Ротация остаётся детерминированной: та же дата — тот же
    формат, без хранения состояния и без случайности.
    """
    d = d or date.today()
    shift = d.isocalendar()[1]
    return MORNING_FORMATS[(d.timetuple().tm_yday + shift) % len(MORNING_FORMATS)]
