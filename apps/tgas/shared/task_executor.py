...
1. Сначала реши, твоя ли это задача. Если она относится к другому отделу —
   ...
   Себе передать нельзя, и после двух передач инструмент откажет: тогда
   задача твоя окончательно — сделай её или заведи human_task.
...
3. НИКОГДА не называй цены, остатки, суммы и имена клиентов по памяти.
   ...
   Цена продажи и цена закупки — РАЗНЫЕ ЧИСЛА. Прайс отвечает только на
   «почём продаём»; для закупки бери avg_cost из get_inventory, а если его
   нет — так и скажи, вместо подстановки розничной цены.
...
async def execute_bot_task(
    ...
    try:
        result = await tool_runtime.run_tool_loop(
            ...
            #
            # caller_department — чтобы делегирование могло отказать в передаче
            # самому себе. Без него отдел не знает, кто он, и Стёпан 10.08.2026
            # передавал задачу «в отдел pm», то есть себе, бесконечно.
            extra_args={
                "task_id": task_id,
                "hops": hops,
                "chat_id": chat_id,
...
async def _record_own_performance(bot_name: str, result, escalated: bool) -> None:
    ...
    Три метрики на задачу, 1 — плохо, 0 — хорошо:
      ...
      · no_action — ни одного УДАВШЕГОСЯ вызова: либо ответ голым текстом,
        либо все инструменты отказали. Для обучения это одно и то же —
        работа не сделана.
    """
    try:
        from shared.feedback_loop import feedback_loop
