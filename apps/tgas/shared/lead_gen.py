...
# Клиника, супермаркет, вуз и бизнес-центр отсюда ВЫВЕДЕНЫ (21.08.2026).
# Микрозелень покупает тот, кто сам готовит еду, а ночная ротация исправно
# набивала ими список клиентов и карту: она перебирает все ключи этого
# словаря по одному за ночь, и в свою ночь очередь доходила до «университет».
# Подписи им оставлены ниже — см. комментарий к VENUE_LABELS_RU. Зеркало
# решения: RETIRED_COMPANY_TYPES в apps/web/src/lib/customers/companyTypes.ts.
...
#
# Здесь ключей БОЛЬШЕ, чем в VENUE_QUERIES, и это намеренно: выведенные из
# сбора типы остались у карточек, за которыми есть заказ (клинику, которая у
# нас покупает, чистка не трогает). Без подписи КП такому клиенту ушло бы со
# словом «заведение» — а с ним уходит «клиника».
