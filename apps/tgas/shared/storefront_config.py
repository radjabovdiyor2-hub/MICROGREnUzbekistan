...
#
# ⚠️ ЗДЕСЬ ОБЕЩАЛИСЬ CLICK И PAYME. Способы оплаты убраны с оформления,
# потому что платёж не создавался: merchant ID были заглушками, и кнопка
# «Оплатить» вела в никуда. На витрине дефолт починили
# (`['cash', 'card', 'transfer']`), а эта копия осталась прежней — и
# срабатывает она ровно тогда, когда витрина недоступна: база знаний
# собирается на запасных значениях, и бот начинает предлагать клиентам
# оплату, которой нет. `check_prompts.py` этого не ловил: правило про
# Click и Payme смотрит промпты, а здесь обычный словарь.
