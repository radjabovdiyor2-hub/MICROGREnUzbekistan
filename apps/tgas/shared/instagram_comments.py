...
# Телефон подставляется из настроек (`settings.company_phone` → BRAND["phone"]).
# Вписанный сюда строкой, он ровно так и разошёлся в прошлый раз: .env был
# заполнен верно, а промпт диктовал клиенту заглушку.
REPLY_SYSTEM = (
    ...
    f"{settings.company_phone}. Без длинных списков."
)
...
async def _gen_reply(comment_text: str) -> str:
    try:
        ...
        return (
            f"Спасибо за интерес! Напишите нам в Директ или на "
            f"{settings.company_phone} — всё расскажем 🌱"
        )
