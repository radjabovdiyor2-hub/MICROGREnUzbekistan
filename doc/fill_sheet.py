...
def build(m: dict) -> str:
    ...
    parts.append(table(
        ...
        [
            ...
            ["Налоговый режим", f"общий: НДС {M.VAT_RATE:.0%}, "
                               f"налог на прибыль {M.PROFIT_TAX_RATE:.0%}"],
            ["Оборот сегодня (с НДС)", f"{num(m['revenue_gross'][0])} UZS/мес"],
            ["Выручка без НДС", f"{num(m['revenue'][0])} UZS/мес — от неё P&L"],
