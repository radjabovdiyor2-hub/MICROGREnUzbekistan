...
**31 групп, 131 route-файлов.** Список ниже сверен с каталогом
...
| POST | `/api/admin/tracking/ping` | Крошки трека полевого сотрудника, пачкой. Две двери: Telegram-бот с общим секретом и `telegramId`, либо PWA админки с сессией (STAFF). Кто это — из подписи, а не из тела |
| POST | `/api/admin/tracking/photo` | Фотоотчёт со стоянки: сжимается до 1600 px по длинной стороне и привязывается к `track_stays`. Две двери: сессия продавца со `stayId`/`clientRef` либо бот с `telegramId` — тогда открытую стоянку находит сервер. Отдельно от `/api/upload`: тот не сжимает и ни к чему не привязывает |
| POST/PATCH | `/api/admin/tracking/stay` | «Я на точке» (POST, возвращает `stayId` для фото) и «уехал» (PATCH, считает длительность). Две двери: сессия продавца называет клиента сама, бот с `telegramId` — тогда клиента выбирает СЕРВЕР по последней крошке трека в радиусе `field.stayRadiusM`. Стоянка помечается `manual` — отдельно от выведенной по треку |
| GET | `/api/admin/tracking/live` | Кто сейчас в поле: трек с начала смены, последняя точка и сколько минут молчит. Только владельцу — видеть коллегу продавцу незачем |
| GET | `/api/admin/tracking/silent` | Кто сегодня не на связи: не включал трансляцию или замолчал дольше 15 минут. Для сторожа в `sales_bot` — общий секрет бота. Отвечает фактом, решение о напоминании принимает бот |
| GET | `/api/admin/tracking/day` | День в поле: трек, стоянки с длительностью, фото, плечи с фактом и эталоном по пробкам. Владелец видит любой день, продавец — только свой |
| POST | `/api/admin/deliveries/accept` | «Приступить» по рейсу. Водитель опознаётся по `telegramId`, принадлежность рейса проверяет сервер: кнопку можно переслать |
| POST | `/api/admin/visit-plans/summarize` | Вечерний итог объездов: «закончен», «не начат», «отмечено 3 из 8» — в колокольчик владельцу. Зовёт сторож офиса в 19:00; сигнал пишется раз на пару «день + человек» |
| GET | `/api/admin/staff/me` | Кто это по `telegramId` и что у него на сегодня. Бот один для покупателя и сотрудника, а меню им нужно разное; связка Telegram ↔ сотрудник живёт только здесь. «Не сотрудник» — ответ, а не ошибка |
| GET | `/api/admin/staff/day` | Готовый текст «Мой день» и ссылка навигации по НЕПРОЙДЕННЫМ точкам. Текст собирает витрина: тот же список печатает уведомление о назначении |
| POST | `/api/admin/visit-plans/accept` | «Приступить»: сотрудник подтвердил, что увидел объезд. Дверь для бота — общий секрет плюс `telegramId`; чей это план, решает СЕРВЕР, а номер из кнопки проверяется на принадлежность. Подтверждение, а не шлагбаум |
| DELETE | `/api/admin/visit-plans` | Снять объезд дня: `?date=&assignee=`. Владелец снимает любой, продавец — только свой (имя из подписи). Отметки визитов остаются: поездка не перестаёт быть фактом оттого, что план отменили |
...
| POST/GET/HEAD | `/api/farm/frame` | Живой кадр из теплицы. POST кладёт (закрыт `BOT_SECRET`: снимать вправе только то, что стоит на ферме), GET отдаёт витрине, HEAD отвечает «есть ли кадр» без самого кадра. Хранится ОДИН последний снимок и только пока он свежий — пять минут; дальше 404, и блок на главной гаснет. Показать вчерашний кадр под словом «сейчас» — та же ложь, из-за которой переделан прошлый блок фермы. Кладёт кадры `scripts/farm-camera.mjs`, запускаемый рядом с камерой: RTSP у EZVIZ живёт только в локальной сети |
