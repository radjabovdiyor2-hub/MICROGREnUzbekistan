...
| GET | `/api/admin/tracking/live` | Кто сейчас в поле: трек с начала смены, последняя точка, сколько минут молчит и с какой минуты считать молчание (`silentAfterMin`). Только владельцу — видеть коллегу продавцу незачем |
