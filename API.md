...
| POST/PATCH | `/api/admin/tracking/stay` | «Я на точке» (POST, возвращает `stayId` для фото) и «уехал» (PATCH, считает длительность). Стоянка помечается `manual` — отдельно от выведенной по треку (STAFF, только своя) |
