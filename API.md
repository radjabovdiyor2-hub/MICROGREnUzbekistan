...
**33 группы, 136 route-файлов.** Список ниже сверен с каталогом
...
| GET | `/api/admin/tracking/live` | Кто сейчас в поле: трек с начала смены, ПОСЛЕДНЯЯ (самая свежая) точка, сколько минут молчит и с какой минуты считать молчание (`silentAfterMin`). Рядом — `points` (крошек за день), `lastSource` (`app`/`pwa`/`telegram_live`) и `trimmed`: по ним видно, почему трек редкий, и что показан лишь последний отрезок. `?employee=<id>` — тот же ответ про одного: на этом стоит экран слежения. Владельцу по сессии ИЛИ боту по общему секрету (сводка «кто в поле» в Telegram); продавцу — нет, видеть коллегу ему незачем |
| POST/GET/DELETE | `/api/auth/device` | Ключ устройства для приложения Android. POST — сотрудник выдаёт себе ключ, войдя по PIN (показывается ОДИН раз, в базе только отпечаток). GET и DELETE — владельцу: список телефонов и отзыв. Ключ открывает ровно две двери: свой трек и свою смену |
| GET | `/api/app/version` | Какая версия приложения считается свежей и куда за ней идти. Задаёт владелец настройками `field.appVersion` / `field.appUrl`; пусто — приложение молчит |
| GET | `/api/admin/tracking/silent` | Кто сегодня не на связи И КТО СТОИТ НА МЕСТЕ. `state`: `off` (смены нет — сторож молчит вовсе), `never` (смена открыта, точек нет), `silent` (точки прекратились), `ok`. Отдельным полем `idleMin` — сколько минут человек стоит не у клиента, будучи на связи: это другой вопрос и другой разговор. Пороги отдаются рядом (`silentAfterMin`, `idleAfterMin`), чтобы бот не держал их копий. Для сторожа в `sales_bot` — общий секрет бота. Отвечает фактом, решение о напоминании принимает бот |
| GET | `/api/admin/tracking/day` | День в поле: трек, стоянки с длительностью, фото, плечи с фактом и эталоном по пробкам, простои (`idle`) и разрывы связи (`gaps`). Простои считаются при чтении, а не хранятся: трек дописывается задним числом, и сохранённое разошлось бы с линией на карте. Владелец видит любой день, продавец — только свой |
...
| GET/POST | `/api/shift` | СВОЯ смена сотрудника: `GET` — открыта ли и с какого времени, `POST {action:'open'\|'close'}` — открыть или закрыть. Две дороги внутрь: сессия сотрудника (приложение, веб) или `BOT_SECRET` плюс `telegramId` (Telegram). Кто это — из подписи, а не из тела. Повторное открытие возвращает уже открытую смену: человек жмёт кнопку в боте, не увидев, что нажал её в приложении, и ошибка здесь пугала бы там, где всё в порядке. Не путать с `/api/admin/shifts` — тот график, его составляет владелец |
| POST | `/api/farm/pull` | Снять кадр с камеры теплицы через облако EZVIZ — БЕЗ компьютера на ферме. Зовёт сторож офиса раз в минуту (`BOT_SECRET`): у витрины своего планировщика нет. Не настроено — спокойное `skipped`; камера выключена — `failed` с причиной словами, но 200: ночью камера не работает, и это не поломка сервиса. Облако EZVIZ отвечает 200 НА ВСЁ, различая успех и отказ полем внутри тела, — код, проверяющий только статус HTTP, молча перестал бы снимать |
