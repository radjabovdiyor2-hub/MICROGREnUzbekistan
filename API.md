...
**30 групп, 124 route-файлов.** Список ниже сверен с каталогом
...
| POST | `/api/admin/tracking/photo` | Фотоотчёт со стоянки: сжимается до 1600 px по длинной стороне и привязывается к `track_stays`. Отдельно от `/api/upload` — тот не сжимает и ни к чему не привязывает (STAFF) |
