...
**Схемой владеет Prisma** — `packages/database/prisma/schema.prisma`, 85 моделей.
...
| Model | Purpose |
|-------|---------|
| `VisitPlan`, `VisitPlanStop` | Объезд на дату: кому назначен и какие точки, в каком порядке |
| `VisitPlanItem` | Что взять с собой в объезд. Список НЕОБЯЗАТЕЛЕН: объезд бывает развозной и разведочный. Количество — загрузка машины на день, а не отгрузка клиенту: сколько кому ушло, знает чек кассы |
| `VisitSchedule` | Регулярный день заезда к клиенту («по субботам»). День недели в ISO: пн = 1, вс = 7 — `Date.getDay()` считает воскресенье нулём, и смешение шкал уводит объезд на день назад |

Расписание НЕ создаёт план: оно подсказывает, кого поставить в объезд на
дату, а собирает его человек. Исполнение считается по отметкам визитов
(`interactions` с типами `visit_*`), а не хранится в плане, — иначе объезд
можно было бы закрыть из кресла.
