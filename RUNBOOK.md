...
## 6a. Подключение торговой площадки

Код готов ко всем каналам из реестра (`apps/web/src/lib/channels/registry.ts`),
но включать канал в одиночку он не имеет права: сначала договор, потом
рубильник. Порядок один и тот же.

**1. Подтвердить домен.** Токен из кабинета кладётся в `.env` витрины,
код уже читает его и выводит метку в `<head>`:

| Площадка | Переменная | Где взять |
|---|---|---|
| Google Search Console / Merchant Center | `NEXT_PUBLIC_GSC_VERIFICATION` | Search Console → способ «HTML-тег» |
| Яндекс.Вебмастер | `NEXT_PUBLIC_YANDEX_VERIFICATION` | Вебмастер → «Мета-тег» |
| Meta (каталог для рекламы) | `NEXT_PUBLIC_FACEBOOK_DOMAIN_VERIFICATION` | Business Manager → Бренд-безопасность → Домены |

Пустое значение = метки нет вовсе, и это правильно: метка с пустым токеном
читается площадкой как непройденная проверка, а не как «ещё не настроено».
После правки `.env` нужен пересбор витрины — `NEXT_PUBLIC_*` попадают в
страницу на сборке.

**2. Отдать фид.** Адреса постоянные, отдаются без ключа:
`/feed/google.xml` (Merchant Center), `/feed/meta.csv` (каталог Meta),
`/feed/agents.json` (ИИ-витрины). Пока канал выключен, фид отвечает пустым
списком — это не поломка, а его выключенное состояние.

**3. Включить канал** в админке, вкладка «Каналы продаж»: города доставки,
наценка под комиссию площадки, буфер остатка, окно отгрузки. Для площадки,
которая шлёт заказы к нам, — адрес приёма (`apiUrl`) и общий секрет.

**4. Связать каталог** кнопкой в карточке канала. Скоропорт на складскую
площадку не заводится намеренно: он испортится раньше, чем доедет.

**5. Проверить, что пошло.** В карточке канала — время последней
синхронизации, размер очереди и последняя ошибка текстом. Очередь толкает
Стёпан каждые пять минут (`channels_sync`); руками — кнопка
«Синхронизировать сейчас».

Заказ с площадки принимает `POST /api/channels/<код>/orders` с заголовком
`x-bot-secret`. Повтор того же `externalId` заказ не удваивает — площадки
шлют вебхуки по нескольку раз, это норма их работы, а не сбой.

Разбор чужих форматов заказов (`lib/channels/adapters/`) не написан
намеренно: до договора формат неизвестен, а выдуманный сломался бы на
первом живом заказе. Дверь принимает канонический вид — адаптер пишется,
когда придёт документация площадки.
