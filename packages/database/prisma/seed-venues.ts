...
//
// Читает ВЫГРУЗКУ СБОРЩИКА, а не написанный руками список:
//
//   cd apps/tgas
...
// нём настоящие, от провайдеров; выдуманных заведений здесь быть не должно
...
async function main(): Promise<void> {
  ...
  if (!fs.existsSync(file)) {
    // Молча выходить нельзя: «сид отработал, клиентов нет» — самый
    // дорогой вид тишины. Говорим, чем именно наполнить файл.
    throw new Error(
      `Файл выгрузки не найден: ${file}\n` +
        'Сначала соберите справочник:\n' +
