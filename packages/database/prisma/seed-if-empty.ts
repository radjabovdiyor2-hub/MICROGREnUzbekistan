...
async function run() {
  // ── Справочник сырья — ВСЕГДА, отдельно от каталога ──
  //
  // Проверка «каталог пуст» тут не подходит: на рабочем проде каталог не пуст,
  // и весь блок ниже пропускается, а справочник сырья остался бы пустым.
  //
  // До 10.08.2026 позиции сырья не засевались нигде: их заводили руками, и
  // на проде оказались одни семена гороха — закупки остальных культур
  // не числились нигде. Остатки сидер ставит нулевые: сколько лежит на
  // складе, знает только владелец.
  ...
  // ── Карточка вышедшего номера журнала — ВСЕГДА, по той же причине ──
  //
  // Файлы номера едут в образе (`apps/web/public/magazine/<slug>.{html,pdf}`),
  // а витрина журнала показывает не файл, а карточку из базы. Без этой строки
  // после выкатки раздел говорил бы «номер готовится», хотя номер лежит рядом.
  //
  // Сидер заводит номер ТОЛЬКО когда номеров нет вовсе: как только владелец
  // ведёт их сам, деплой в этот список не вмешивается и удалённый номер не
  // воскрешает. У существующего обновляет лишь пути к файлам.
  runSeed('./seed-magazine-issue.ts');
  ...
  // ── Наборы — ВСЕГДА, и ОБЯЗАТЕЛЬНО ПОСЛЕ импорта каталога ──
  //
  // Набор — это подборка товаров, а не товар: строки в прайсе у него нет
  // намеренно (см. doc/product-sets-spec.md §2.5). Живёт он рецептом, и его
  // ингредиенты ссылаются на товары по слагу — значит товары к этому моменту
  // должны существовать. Поставь эту строку выше импорта, и все двадцать
  // наборов молча превратятся в текст без кнопки «собрать набор»: сидер не
  // падает, он просто не находит товары.
  //
  // Идемпотентен: upsert по slug, шаги и ингредиенты пересоздаются. Фото
  // проставляется только для тех наборов, чей файл реально лежит в
  // apps/web/public/catalog — положили снимки, следующая выкатка их подхватит.
  runSeed('./seed-recipe-sets.ts');
}
