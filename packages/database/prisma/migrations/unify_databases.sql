...
-- ── Шаг 5: снять устаревший CHECK на тип заведения ──────────────────────
--
-- `init.sql` (ныне отставной) заводил ограничение на шесть значений:
--   'restaurant', 'cafe', 'hotel', 'catering', 'retail', 'other'
-- Prisma CHECK-констрейнты не ведёт, поэтому `db push` его не обновляет и
-- не удаляет — он пережил переход на Prisma в живой базе и 20.08.2026
-- уронил выкатку справочника заведений: 'bakery' в список не входил.
--
-- Справочник вырос до девятнадцати типов (`companyTypes.ts`): тойхона,
-- банкетный зал, чайхана, столовая, фастфуд, кофейня, пекарня, фитнес,
-- спорт, санаторий, клиника, супермаркет, вуз, офис.
--
-- Ограничение СНИМАЕМ, а не переписываем списком из девятнадцати значений:
-- иначе тот же список живёт в четвёртом месте (TypeScript, Python,
-- документация, SQL) и снова разойдётся молча — только теперь падением
-- прода. Значения проверяет `isCompanyType()` на каждом пути записи:
-- в API-роуте клиентов, в фильтрах списка и карты.
ALTER TABLE customers DROP CONSTRAINT IF EXISTS customers_company_type_check;
